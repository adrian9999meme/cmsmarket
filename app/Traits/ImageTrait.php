    ...
    public function saveImage($requestImage, $for = '_product_', $save_to_db = false,$url=null,$token_id=null)
    {
        ...
        if ((!empty($requestImage) && $requestImage != 'null') || $url) :
            ...
            elseif ($for == 'admin_light_logo' || $for == 'admin_dark_logo' || $for == 'footer_logo' || $for == 'invoice_logo' ||
                ...
                $for == 'store_logo' || $for == 'store_banner' || $for == 'store_main_banner'):
                ...
                elseif ($for == 'store_logo'):
                    $image_320x320       = date('YmdHis') . "-" . $for . '-320x320' . rand(1, 50) . '.jpg';
                    $image_72x72        = date('YmdHis') . "-" . $for . '-72x72' . rand(1, 50) . '.jpg';
                ...
                elseif ($for == 'store_banner'):
                    $image_1900x625       = date('YmdHis') . "-" . $for . '-1900x625' . rand(1, 50) . '.jpg';
                    $image_72x72        = date('YmdHis') . "-" . $for . '-72x72' . rand(1, 50) . '.jpg';

                    $image_1900x625_url  = $directory . $image_1900x625;
                    $image_72x72_url    = $directory . $image_72x72;

                    Image::make($requestImage)->resize(1900, 685,
                        function ($constraint) {
                            $constraint->aspectRatio();
                        })->save('public/'.$image_1900x625_url,$encode_percentage);
                ...
                elseif ($for == 'store_main_banner'):
                    $image_1300x400       = date('YmdHis') . "-" . $for . '-1300x400' . rand(1, 50) . '.jpg';
                    $image_72x72        = date('YmdHis') . "-" . $for . '-72x72' . rand(1, 50) . '.jpg';

                    $image_1300x400_url  = $directory . $image_1300x400;
                    $image_72x72_url    = $directory . $image_72x72;

                    Image::make($requestImage)->resize(1300, 400,
                        function ($constraint) {
                            $constraint->aspectRatio();
                        })->save('public/'.$image_1300x400_url,$encode_percentage);
                ...
                $images = array(
                    ...
                    'image_1900x625'     => $image_1900x625_url ?? '',
                    'image_1300x400'     => $image_1300x400_url ?? '',
                );
