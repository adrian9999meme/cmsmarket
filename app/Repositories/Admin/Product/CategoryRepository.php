...
class CategoryRepository implements CategoryInterface
{
    ...
    {
        if($lang == null):
            $catByLang = CategoryLanguage::with('category')->where('lang', \App::getLocale())->where('category_id', $id)->first();
        else:
    ...
    {
        return $this->all()->with('childCategories')->where('lang', \App::getLocale())->where('stores_id', '=', $request->store_id)
            ->when($request->q != null, function($query) use ($request){
                $query->whereHas('categoryLanguage', function ($q) use ($request){
                    $q->where('title', 'like', '%'.$request->q.'%');
                });})->orderBy('parent_id', 'ASC')->orderBy('position', 'ASC')->get();
                // ->paginate($limit);


        // return $this->all()->with('childCategories')->latest()->where('lang', \App::getLocale())
        // ->when($request->q != null, function($query) use ($request){
        //     $query->whereHas('categoryLanguage', function ($q) use ($request){
        //         $q->where('title', 'like', '%'.$request->q.'%');
    ...
    public function store($request)
    {
            ...
            if ($request->category != "") { // Check if the parent root category is selected.
...
//            $category->is_digital        = $request->is_digital;
    ...
    public function productCategories($store_id)
    {
        return Category::with(['categories' => function ($query){
            $query->where('status',1);
        }])->where('status',1)->where('stores_id', $store_id)->where('parent_id',null)->latest()->get();
    }
