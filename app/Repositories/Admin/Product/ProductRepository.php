...
class ProductRepository implements ProductInterface
{
    ...
    {
        // return ProductLanguage::all();
        return ProductLanguage::with('product.thumbnailMedia')->whereHas('product', function ($q) use ($store_id) {
                $q->where('stores_id', $store_id);
            })
            //->where('lang', \App::getLocale())
            ->get();
    }
    
    // public function getByStoreName($store_name)
    // {
    //     return ProductLanguage::with('product.thumbnailMedia')->whereHas('product', function ($q) use ($store_name) {
    //             $q->where('stores_id',StoreProfile::where('slug', $store_name)->value('id'));
    //         })
    //         //->where('lang', \App::getLocale())
    //         ->get();
    // }
public function getByStoreName($store_name, $perPage = 20)
{
    $query = ProductLanguage::with('product.thumbnailMedia')
        ->whereHas('product', function ($q) use ($store_name) {
            $q->where('stores_id', StoreProfile::where('slug', $store_name)->value('id'));
        });

    $total = $query->count(); // Get total product count
    $products = $query->paginate($perPage); // Paginate the results
    ...
    public function paginate($request, $status, $limit, $product_for = null)
    {
        $products = Product::with('createdBy', 'productLanguages','user','stock')
            ->where('stores_id', 'like', '%'. $request->store_id. '%')
