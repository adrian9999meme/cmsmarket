...
class AttributeRepository implements AttributeInterface
{
    ...
    {
        if($lang == null):
            $attributeByLang = AttributeLanguage::with('attribute')->where('lang', \App::getLocale())->where('attribute_id', $id)->first();
        else:
    ...
    {
        return $this->all()->latest()->where('lang', \App::getLocale())->paginate($limit);
    }
    ...
    {
        return ProductStock::where('variant_ids','LIKE','%'.$id.'%')->where('product_id',$product_id)->get();
    }
