...
class CampaignRepository implements CampaignInterface
{
    ...
    {
        if($lang == null):
            $campaignByLang = CampaignLanguage::with('campaign', 'campaign.campaignProducts','campaign.campaignProducts.product')->where('lang', \App::getLocale())->where('campaign_id', $id)->first();
        else:
