...
class OrderRepository implements OrderInterface
{
    ...
    public function paginate($request, $limit)
    {
        ...
        $orders = Order::with(['orderDetails.product', 'user'])
            ...
            // Filter by seller
            ->when($request->filled('sellerId'), function ($query) use ($request) {
                $query->where('seller_id', $request->sellerId);
            })

            // Filter by delivery status
            ->when($request->filled('deliveryStatus'), function ($query) use ($request) {
    ...
    //                ->where('name', $order->variation)
    //                ->first();
    //            if ($product_stock) :
    //                $product_stock->current_stock += $order->quantity;
    //                $product_stock->save();
    //            endif;
    //        endforeach;
    //    }
