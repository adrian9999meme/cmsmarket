...
class StoreCategoryRepository implements StoreCategoryInterface
{
    ...
    {
        if($lang == null):
            $catByLang = StoreCategoryLanguage::with('storeCategory')->where('lang', \App::getLocale())->where('store_category_id', $id)->first();
        else:
...
//            $category->is_digital        = $request->is_digital;
    ...
    {
        return StoreCategory::with(['categories' => function ($query){
            $query->where('status',1);
