...
class StoreFrontController extends Controller
{
    ...
    public function getContent(Request $request,BrandInterface $brands)
    {
        try {
            $data = [
                ...
                'shop_content'  => $request->shop_content == 1 ? 1 : 0,
                'brands'        => ($request->type == 'banner-image' || $request->type == 'banner') && $request->mobile == 1  ? $brands->all()->where('lang', \App::getLocale())->where('status',1)->get() : [],
            ];
    ...
    public function updateHomeContent(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            $this->page->updateHomeContent($request);
    ...
    public function menuSetting(Request $request)
    {
        ...
        return view('admin.store-front.menu',compact('languages','lang','menu_language'));
    ...
    public function link(Request $request)
    {
        $languages = $this->languages->all()->orderBy('id', 'asc')->get();
        $lang = $request->lang ? : App::getLocale();
        $menu_language  = headerFooterMenu('footer_menu',$lang);

        return view('admin.store-front.link',compact('lang','languages','menu_language'));
    }
    ...
    public function bannersUpdate(Request $request)
    {
        ...
        try {
            ...
            if ($request->affiliate_sing_up_banner != null) {
    ...
    public function blankPageUpdate()
    {
        $languages = $this->languages->all()->orderBy('id', 'asc')->get();
        return view('admin.store-front.blank-page', compact('languages',));
    }
    public function careersUpdate()
    {
        $languages = $this->languages->all()->orderBy('id', 'asc')->get();
        return view('admin.store-front.career-page', compact('languages',));
    }
    public function privacyPolicyUpdate()
    {
        $languages = $this->languages->all()->orderBy('id', 'asc')->get();
        return view('admin.store-front.privacypolicy-page', compact('languages',));
    }
    public function latestNewsUpdate()
    {
        $languages = $this->languages->all()->orderBy('id', 'asc')->get();
        return view('admin.store-front.latestnews-page', compact('languages',));
    }
    public function faqUpdate()
    {
        $languages = $this->languages->all()->orderBy('id', 'asc')->get();
        return view('admin.store-front.faq-page', compact('languages',));
    }
    public function helpSupportUpdate()
    {
        $languages = $this->languages->all()->orderBy('id', 'asc')->get();
        return view('admin.store-front.helpsupport-page', compact('languages',));
    }
    public function ourAppUpdate()
    {
        $languages = $this->languages->all()->orderBy('id', 'asc')->get();
        return view('admin.store-front.ourapp-page', compact('languages',));
    }
    public function termsConditionsUpdate()
    {
        $languages = $this->languages->all()->orderBy('id', 'asc')->get();
        return view('admin.store-front.termsconditions-page', compact('languages',));
    }
    public function contactUsUpdate()
    {
        $languages = $this->languages->all()->orderBy('id', 'asc')->get();
        return view('admin.store-front.contactus-page', compact('languages',));
    }
    ...
    public function headerMenuUpdate(Request $request)
    {
        ...
        if ($request->has('label')):
            ...
            $request['site_lang'] = $request->lang;
            unset($request['label']);
            unset($request['url']);
    ...
    public function deletePaymentBanner(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            $this->setting->deleteBanner($request);
            Toastr::success(__('Created Successfully'));
            DB::commit();
            return redirect()->route('other.pages');
        } catch (\Exception $e) {
    ...
    public function statusChange(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            $this->page->statusChange($request['data']);
