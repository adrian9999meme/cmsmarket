...
class StoresCategoryController extends Controller
{
    protected $category;
    protected $catLang;
    protected $languages;

    public function __construct(StoreCategoryInterface $category, StoreCategoryLanguageInterface $catLang, LanguageInterface $languages)
    {
    ...
    public function index(Request $request)
    {
        try {
            ...
            return view('admin.stores.categories.index', compact('categories','all_categories'));
        } catch (\Exception $e) {
    ...
    public function store(CategoryRequest $request)
    {
        ...
        $validated = $request->validate([
            'title' => 'required|unique:store_category_languages',
        ]);
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('stores.categories', ['q' => '', 'mode' => 'create', 'parent' => $request->get('category')]);
        } catch (\Exception $e) {
            DB::rollBack();
            Toastr::error($e->getMessage());
            return redirect()->back();
    ...
    public function edit($id, Request $request)
    {
        try {
            ...
            if ($category_language = $this->category->getByLang($id, $lang)):
                ...
                return view('admin.stores.categories.update', compact('category_language','all_categories', 'languages', 'lang','r'));
            else:
