...
class ProductController extends Controller
{
    ...
    public function index(Request $request, $status = null){
        try {
            ...
            $categories         = $this->categories->allCategory()->where('stores_id', $request->store_id)->where('parent_id', null)->where('status',1);
            $category_id        = $request->category_id;

            return view('admin.products.products.index', compact('status','products','selected_category','selected_seller', 'stores', 'store_id', 'categories', 'category_id'));
        } catch (\Exception $e) {
    ...
    public function create(Request $request)
    {
        $data = [
            'categories'    => $this->categories->allCategory()->where('stores_id', $request->store_id)->where('parent_id', null)->where('status',1),
            'brands'        => $this->brands->all()->where('lang', \App::getLocale())->where('status',1)->get(),
            'colors'        => $this->colors->all()->where('lang', \App::getLocale())->get(),
            'attributes'    => $this->attributes->all()->where('lang', \App::getLocale())->get(),
            'stores'         => $this->stores->all()->get(),
    ...
    public function getAttributeValues(Request $request)
    {
        ...
        if (!empty($attributes_sets)):
            $attributes = $this->attributes->all()->whereIn('attributes.id', $attributes_sets)->where('lang', \App::getLocale())->get();
