...
class CategoryController extends Controller
{
    ...
    public function store(CategoryRequest $request)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('categories',['store_id' => $request->get('store'), 'q' => '', 'mode' => 'create', 'parent' => $request->get('category')]);
        } catch (\Exception $e) {
            DB::rollBack();
            Toastr::error($e->getMessage());
            return redirect()->back();
    ...
    public function edit($id, Request $request)
    {
        try {
            ...
            if ($category_language = $this->category->getByLang($id, $lang)):
                ...
                $store=$store_id;
                return view('admin.products.categories.update', compact('category_language','all_categories', 'languages', 'lang','r','store_id'));
            else:
