...
use App\Repositories\Interfaces\Admin\Addon\AffiliateInterface;
...
class ProductController extends Controller
{
    ...
    public function productDetails(Request $request,$slug, ColorInterface $color, AttributeInterface $attribute): \Illuminate\Http\JsonResponse
    {
        ...
        try {
            ...
            if (count($attributes) > 0) {
                $attribute_selector += count($attributes);
            }
    ...
    public function variants(Request $request, AttributeInterface $attribute, ColorInterface $color): \Illuminate\Http\JsonResponse
    {
        try {
            $i = 0;
            $variant = $request->color_id;

            foreach (array_filter($request->variant_ids) as $key => $value) {
                if ($request->color_id) {
                    $variant .= '-' . $value;
                } elseif ($i == 0) {
                    $variant .= $value;
            ...
            $stocks = $attribute->stocks($variant, $request->product_id);
            ...
            foreach ($stocks as $stock) {
                if ($stock->current_stock > 0)
                {
                    if ($i > 0)
                    {
                        $allowed_attributes[] = (int)str_replace($variant,'',$stock->variant_ids);
                    }
                    else
                    {
                        $allowed_attributes[] = (int)str_replace($variant.'-','',$stock->variant_ids);
    ...
    public function findVariants(Request $request, AttributeInterface $attribute): \Illuminate\Http\JsonResponse
    {
        try {
            ...
            if ($stock) {
                $images = [
                    'image_72x72'     => @get_media($stock->image['image_72x72']),
                    'image_320x320'   => @get_media($stock->image['image_320x320'])
                    // 'image_72x72'     => @is_file_exists($stock->image['image_72x72'], $stock->image['storage']) ? @get_media($stock->image['image_72x72'], $stock->image['storage']) : static_asset('storage/images/default/default-image-72x72.png'),
                    // 'image_320x320'   => @is_file_exists($stock->image['image_320x320'], $stock->image['storage']) ? @get_media($stock->image['image_320x320'], $stock->image['storage']) : static_asset('storage/images/default/default-image-320x320.png')
                ];
            }
...
//                'colors'            => $color->shopColors(),
    ...
    public function filterByKeyWord(Request $request)
    {
        try {
            ...
            //$response = Http::asJson()->post('http://127.0.0.1:5000/process_keyword', [
	    //    'keyword' => $keyword
	    //]);    
            //if ($response->successful()) {
            //    $pluralWord = $response->json()['plural'];
            //} else {
            //    return response()->json(['error' => 'Python API failed'], 500);
            //}
	    $pluralWord=$keyword;
            $solrUrl = 'http://localhost:8984/solr/lekit_core/select?';

            $queryParams = [
                'q' => '*:*',
                'rows' => $perPage,
                'wt' => 'json',
                'defType' => 'edismax', // Set the defType to edismax
                'pf' => 'category1^5', // Phrase fields with boosting
                'qf' => 'category1^5', // Query fields with boosting
                // 'stopwords' => 'true', // Enable stopwords
            ];
            ...
            if (!empty($keyword)) {
                ...
                $jsonPath = base_path('last_result.json');
                if (!file_exists($jsonPath)) {
                    return response()->json(['error' => 'result.json not found'], 500);
                }

                $jsonData = json_decode(file_get_contents($jsonPath), true);

                // Loop through array and check if plural word exists
                $matchFound = false;
                $matchFound1 = false;
                foreach ($jsonData as $item) {
                    $parts = preg_split('/[\/>]/', $item);
                    // $parts = array_map('trim', $parts); // Trim spaces
                    $parts = array_map('strtolower', $parts); // Lowercase for comparison

                    // Check if plural word is in any part
                    if (in_array($pluralWord, $parts)) {
                        $matchFound = true;
                    }
                ...
                if($matchFound)
                    $queryParams['q'] = 'category1_exact:(' . $pluralWord . ')';
                if(!$matchFound && $matchFound1)
                    $queryParams['q'] = 'category1_exact:("' . $keyword . '")';
                if(!$matchFound && !$matchFound1)
                    $queryParams['q'] = 'category1:(' . $keyword . ') OR title:(' . $keyword . ')';
                // Boost results where category contains the keyword
            }

            if (!empty($category)) {
                $category = strtolower(trim($category));
                $queryParams['q'] = 'category1:(' . $category . ')';
            }

            $queryString = http_build_query($queryParams);
            $response = file_get_contents($solrUrl . $queryString);
            ...
            return response()->json($solrResponse);
    ...
    public function recommendProducts(Request $request)
    {
        try {
            ...
            $products = NewProduct::inRandomOrder()->limit(4)->get();
    
            return response()->json($products);
        } catch (\Exception $e) {
            // Log the error message for debugging
            \Log::error('Error recommending products: ' . $e->getMessage());
            return response()->json(['message' => 'Server Error'], 500);
        }
    }


    public function productsByStoreId(Request $request, ProductInterface $product)
    {
        try {
            $data = [
                'products' => $product->getByStore($request->store_id)
            ];
            return response()->json($data);
        } catch (\Exception $e) {
            return response()->json([
    ...
    // public function productsByStoreName(Request $request, ProductInterface $product)
    // {
    //     try {
    //         $data = [
    //             'products' => $product->getByStoreName($request->store_name)
    //         ];
    //         return response()->json($data);
    //     } catch (\Exception $e) {
    //         return response()->json([
    ...
    public function productsByStoreName(Request $request, ProductInterface $product)
    {
        try {
            ...
            $result = $product->getByStoreName($storeName, $perPage);
    
            return response()->json([
                'products' => $result['products']->items(), // Paginated products
                'next_page_url' => $result['products']->nextPageUrl(), // Next page URL
                'total' => $result['total'] // Total product count
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'error' => $e->getMessage()
    ...
    public function productsById(Request $request, ProductInterface $product)
    {
    ...
    public function fetch(Request $request) {
        ...
        try {
            $response = Http::withOptions([
                'proxy' => 'http://172.167.161.8:8080',
                'verify' => false, // Disable SSL verify for flaky proxies
                'timeout' => 10
            ])->withHeaders([
            ...
            return response('Failed to fetch image via proxy: ' . $e->getMessage(), 500);
        }

        // return response('Image not found or proxy failed.', 404);
    }
    // public function productsByStoreId(Request $request, ProductInterface $product)
    // {
    //     try {
