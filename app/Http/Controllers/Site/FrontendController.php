...
use App\Http\Resources\SiteResource\StorePaginateResource;
...
class FrontendController extends Controller
{
    ...
    public function blankPageContent(Request $request){
        return response()->json(['content' => Setting::where("title", $request->page_title)->latest()->limit(1)->get('value')]);
    }
    ...
    public function productCategories(Request $request, CategoryInterface $category): \Illuminate\Http\JsonResponse
    {
        try {
            $data = [
                'categories' => CategoryResource::collection( $category->productCategories($request->store_id))
            ];
            return response()->json($data);
        } catch (\Exception $e) {
            return response()->json([
    ...
    public function storeCategories(Request $request, StoreCategoryInterface $store_category): \Illuminate\Http\JsonResponse
    {
        try {
            $data = [
                'categories' => CategoryResource::collection( $store_category->allCategories())
            ];
            return response()->json($data);
        } catch (\Exception $e) {
            return response()->json([
    ...
    public function stores(StoreInterface $store,Request $request): \Illuminate\Http\JsonResponse
    {
        try {
            $data = [
                // 'stores' => new StorePaginateResource($store->allStore()->get())
                'stores' => $store->allStore()->get()
            ];
            return response()->json($data);
        } catch (\Exception $e) {
            return response()->json([
    ...
    public function getStore(StoreInterface $store, Request $request) {
        try {
            $data = [
                'store' => $store->getStore($request->id)
            ];
            return response()->json($data);
        } catch (\Exception $e) {
            return response()->json([
    ...
    //test
    public function gallery(MediaInterface $media,Request $request,$store_code): \Illuminate\Http\JsonResponse
    {
        try {
            $data = [
                // 'stores' => new StorePaginateResource($store->allStore()->get())
                'gallery' => $media->gallery($store_code)
            ];
            return response()->json($data);
        } catch (\Exception $e) {
            return response()->json([
    ...
    public function test(MediaInterface $media,Request $request,$store_code): \Illuminate\Http\JsonResponse
    {
        try {
            $data = [
                // 'stores' => new StorePaginateResource($store->allStore()->get())
