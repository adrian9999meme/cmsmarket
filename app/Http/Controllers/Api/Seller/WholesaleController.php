    ...
    public function edit($id, Request $request){
        try {
            ...
            if ($product->user_id == authUser($request)->id && $product_language = $this->products->getByLang($id, $lang)):
                ...
                $brands                 = BrandResource::collection($this->brands->all()->where('lang', \App::getLocale())->where('status',1)->get());
                $colors                 = ColorResource::collection($this->colors->all()->where('lang', \App::getLocale())->get());
                $attributes             = AttributeResource::collection($this->attributes->all()->where('lang', \App::getLocale())->get());
