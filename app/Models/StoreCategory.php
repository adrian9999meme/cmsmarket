...
class StoreCategory extends Model
{
    ...
    {
        return $this->belongsToMany(StoreProfile::class,'store_category', 'category_id','store_id');
    }
    ...
    {
        return $this->hasMany(StoreCategory::class, 'parent_id')->with('childCategories');
    }

    public function categoryLanguage(): \Illuminate\Database\Eloquent\Relations\HasMany
    {
        return $this->hasMany(StoreCategoryLanguage::class);
    }
    ...
    {
        return $this->hasOne(StoreProfile::class)->where('status','published')->latest();
    }
    ...
    public function getTranslation($field, $lang = 'en')
    {
        ...
        if (blank($cat_translation)):
            $cat_translation = $this->categoryLanguage()->where('lang', \App::getLocale())->first();
        endif;
