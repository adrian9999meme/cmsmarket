...
class Brand extends Model
{
    ...
    public function getTranslateAttribute()
    {
        ...
        if (!$row)
            $row = $this->currentLanguage->where('brand_id',$this->id)->where('lang', \App::getLocale())->first();
    ...
    public function getTranslation($field, $lang = 'en')
    {
        ...
        if (blank($brand_translation)):
            $brand_translation = $this->hasMany(BrandLanguage::class)->where('lang', \App::getLocale())->first();
        endif;
