...
class BlogCategory extends Model
{
    ...
    public function getTranslation($field, $lang = 'en')
    {
        ...
        if (blank($category_translation)):
            $category_translation = $this->hasMany(BlogCategoryLanguage::class)->where('lang', \App::getLocale())->first();
        endif;
    ...
    {
        return $this->hasMany(BlogCategoryLanguage::class)->where('lang', \App::getLocale());
    }
    ...
    public function getTranslateAttribute()
    {
        ...
        if($this->isTranslationsLoaded())
        {
            ...
            if (!$row)
                $row = $this->currentLanguage()->where('blog_category_id',$this->id)->where('lang', \App::getLocale())->first();
        }
