...
class Page extends Model
{
    ...
    public function getTranslation($field, $lang = 'en')
    {
        ...
        if (blank($page_translation)):
            $page_translation = $this->hasMany(PageLanguage::class)->where('lang', \App::getLocale())->first();
        endif;
    ...
    public function getTranslateAttribute()
    {
        ...
        if (!$row)
            $row = $this->currentLanguage->where('page_id', $this->id)->where('lang', \App::getLocale())->first();
