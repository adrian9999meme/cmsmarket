...
class Coupon extends Model
{
    ...
    public function getTranslation($field, $lang = 'en')
    {
        ...
        if (blank($product_translation)):
            $product_translation = $this->hasMany(CouponLanguage::class)->where('lang', \App::getLocale())->first();
        endif;
    ...
    public function getTranslateAttribute()
    {
        ...
        if (!$row)
            $row = $this->currentLanguage->where('coupon_id',$this->id)->where('lang', \App::getLocale())->first();
