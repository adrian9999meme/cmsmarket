...
class VideoShopping extends Model
{
    ...
    public function getTranslateAttribute()
    {
        ...
        if (!$row)
            $row = $this->currentLanguage->where('video_shopping_id',$this->id)->where('lang', \App::getLocale())->first();
    ...
    public function getTranslation($field, $lang = 'en')
    {
        ...
        if (blank($video_shopping_translation)):
            $video_shopping_translation = $this->hasMany(VideoShoppingLanguage::class)->where('lang', \App::getLocale())->first();
        endif;
