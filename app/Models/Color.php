...
class Color extends Model
{
    ...
    public function getTranslation($field, $lang = 'en')
    {
        ...
        if (blank($color_translation)):
            $color_translation = $this->hasMany(ColorLanguage::class)->where('lang', \App::getLocale())->first();
        endif;
    ...
    public function getTranslateAttribute()
    {
        ...
        if (!$row)
            $row = $this->currentLanguage->where('color_id',$this->id)->where('lang', \App::getLocale())->first();
