...
class Campaign extends Model
{
    ...
    protected $appends = ['image_1920x412','image_406x235','image_374x374','short_description','campaign_start_date','campaign_end_date',];
    protected $casts = [
        'banner' => 'array',
        'thumbnail' => 'array'
    ];
    ...
    public function getTranslateAttribute()
    {
        ...
        if (!$row)
            $row = $this->currentLanguage->where('lang', \App::getLocale())->first();
    ...
    public function getTranslation($field, $lang = 'en')
    {
        ...
        if (blank($campaign_translation)):
            $campaign_translation = $this->hasMany(CampaignLanguage::class)->where('lang', \App::getLocale())->first();
        endif;
