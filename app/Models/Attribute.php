...
class Attribute extends Model
{
    ...
    public function getTranslation($field, $lang = 'en')
    {
        ...
        if (blank($attribute_translation)):
            $attribute_translation = $this->hasMany(AttributeLanguage::class)->where('lang', \App::getLocale())->first();
        endif;
    ...
    public function getTranslateAttribute()
    {
        ...
        if (!$row)
            $row = $this->currentLanguage->where('attribute_id',$this->id)->where('lang', \App::getLocale())->first();
