...
const EmailInbox = () => {
    ...
    //         return false;
    //     }
    //     return displayCategory === "inbox" || displayCategory === category;
    // };
    const [isCheckAll, setIsCheckAll] = useState(false);

    const filterMails = ({ category }) => {
        if (isCheckAll) {
            return true; // Show all items when "Check All" is toggled
        }

        if (displayCategory === "inbox" && category === "trash") {
            return false; // Default filtering logic
        }
        return displayCategory === "inbox" || displayCategory === category;
    };

    const checkFirstCheckbox = () => {
        const checkboxElement = document.querySelectorAll(
    ...
    //     const allChecked = Array.from(checkboxElement).every(
    //         (checkbox) => checkbox.checked
    //     );
    //     const someChecked = Array.from(checkboxElement).some(
    //         (checkbox) => checkbox.checked
    //     );

    //     checkall.checked = allChecked; // Check "Check All" only if all are selected
    //     checkall.indeterminate = !allChecked && someChecked; // Set indeterminate state
    // };

    // search
    const handelSearch = (searchQuery) => {
        const filteredEmails = mailsLists.filter(
            (email) =>
